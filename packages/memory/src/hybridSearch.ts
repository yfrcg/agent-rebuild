/**
 * ?????CS336 ???
 * ???packages/memory/src/hybridSearch.ts
 * ??????????
 * ????????????FTS/?????????????
 * ???????????????????????????????????? README ????????????????
 */
 ...
 *
 * Args:
 *   query: 用户当前问题或系统构造出的检索短语。
 *   limit: 最多返回的记忆片段数量。
 *
 * Returns:
 *   Promise<MemorySearchResult[]>：按融合分数排序后的记忆片段，包含来源、分数和正文。
 *
 * 实现步骤：
 *   1. 先走 FTS 关键词检索，优先拿到精确文本证据。
 *   2. FTS 不可用或结果较弱时，回退到 LIKE 检索兜底。
 *   3. 再走向量检索，补充语义相近但字面不同的记忆。
 *   4. 用 RRF 和时间衰减把两路结果融合排序。
 */
