...
function splitIntoChunks(filePath: string, content: string): MemoryChunk[] {
  ...
  function shouldSkipChunk(section: string, text: string) {
    ...
    // 纯一级标题块，例如 "# MEMORY.md"、"# 2026-04-18"
    if (/^#\s+.+$/.test(trimmed) && !trimmed.includes("\n")) {
      return true;
    }

    // ROOT 块里如果只有很短的标题型内容，也跳过
    if (section === "ROOT" && trimmed.length < 30) {
      return true;
    }
...
export function rebuildMemoryIndex() {
  ...
  const insertEmbedding = db.prepare(`
    INSERT INTO mem_embeddings (chunkId, filePath, section, content, embedding)
    VALUES (@chunkId, @filePath, @section, @content, @embedding)
  `);
  ...
  for (const filePath of files) {
    ...
    for (const chunk of chunks) {
      ...
      insertEmbedding.run({
        ...chunk,
        embedding: null,//把 embedding 字段设为了 null，backfill 负责在后台慢条斯理地把这些 null 的记录挑出来，送给阿里云大模型生成 1024 维的向量
      });
    }
  }
}
