...
export class Gateway {
  ...
  /**
   * 处理一次用户请求，并返回 Gateway 对外承诺的响应结构。
   *
   * Args:
   *   request: 用户输入、会话 ID、权限模式、项目边界、启用 Skill 等请求上下文。
   *   options: 取消信号、事件回调和运行期覆盖项，主要供 WebSocket 与测试使用。
   *
   * Returns:
   *   Promise<GatewayResponse>：最终文本、调试信息、工具轨迹、指标和可能的错误摘要。
   *
   * 实现步骤：
   *   1. 确保 MCP 等懒加载依赖可用。
   *   2. 执行限流、熔断、权限模式等运行时守卫。
   *   3. 把模型循环和工具调用委托给 AgentRunner。
   *   4. 归一化错误、审计、指标和最终响应。
   */
  async handle(
