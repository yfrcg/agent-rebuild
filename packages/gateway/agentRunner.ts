/**
 * ?????CS336 ???
 * ???packages/gateway/agentRunner.ts
 * ???Gateway ?????
 * ??????? Agent ?????????????????????
 * ???????????????????????????????????? README ????????????????
 */
...
export class AgentRunner {
  ...
  /**
   * 运行一次 LLM 控制循环，直到模型给出 final 或达到工具调用上限。
   *
   * Args:
   *   request: 当前用户任务、会话状态、项目边界、权限模式和可选计划状态。
   *   options: 取消信号、流式事件回调、模型/工具循环开关等运行期控制项。
   *
   * Returns:
   *   Promise<AgentRunnerResult>：最终文本、工具调用记录、DevTask 状态、token/cost 指标和调试信息。
   *
   * 实现步骤：
   *   1. 检索记忆并构建分层上下文。
   *   2. 调用模型，要求模型输出 final JSON 或 tool_call JSON。
   *   3. 如果是工具调用，交给 ToolCallExecutor，并把工具证据追加回上下文。
   *   4. 循环直到得到可信 final；开发任务会额外追踪测试、修复轮次和文件变化。
   */
  async run(
