...
import { AgentRunner } from "../packages/gateway/agentRunner";
...
test("AgentRunner executes file.write from mixed plain-text tool-call output", async () => {
  ...
  try {
    ...
    const result = await runner.run(
      createGatewayRequest("帮我设计一个打印杨辉三角的cpp程序", {
        permissionMode: "bypassPermissions",
        projectBoundary: {
          projectDir,
          permission: "project-write",
          allowedReadRoots: [projectDir],
          allowedWriteRoots: [projectDir],
    ...
    assert.equal(result.toolCalls.some((toolCall) => toolCall.toolName === "file.write" && toolCall.status === "success"), true);
  } finally {
    fs.rmSync(workspace, { recursive: true, force: true });
  }
});

test("AgentRunner keeps going after unsupported final and eventually writes the file", async () => {
