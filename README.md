...
npm run web:dev             # 本地 Web UI（默认 Vite 端口）
...
Web UI 会通过 `VITE_GATEWAY_WS_URL` 连接 Gateway；未配置时默认使用 `/v1/ws`，本地开发可用 Vite 代理或显式配置到 `ws://127.0.0.1:8787/v1/ws`。
...
`apps/web-ui` 是本地 Agent Console，提供比 REPL 更完整的运行态可视化：

- 左侧功能区：聊天、概览、资源、审批、记忆、审计入口和最近会话列表。
- 大聊天区：展示用户消息、助手最终回复、流式 token 增量和任务运行过程。
- 过程时间线：展示模型调用、工具调用、工具输出、DevTask、多 Agent/ReviewGraph 状态。
- 右侧事件卡片：默认收起，点击后查看 `run.*`、`chat.*`、`tool.*`、`session.updated` 等实时事件。
- 会话管理：创建、搜索、切换、重命名已有会话，并将会话绑定到任意安全的本机工作目录。
- 运行配置摘要：模型、流式模式、工具循环、多 Agent 开关、工作目录以小字展示在聊天顶部。
...
├── apps/web-ui/                   # React 本地 Agent Console
│   ├── src/App.tsx                # 控制台主界面（左侧会话 + 大聊天区 + 事件卡片）
│   ├── src/providers/             # Gateway WS 连接、事件订阅、状态重同步
│   ├── src/stores/                # Zustand 状态（session/run/event/approval）
│   └── src/styles/tokens.css      # 设计 token 与控制台布局样式
├── packages/
...
│   │   ├── compaction.ts          # 会话压缩
│   │   └── summary.ts             # 会话摘要
│   ├── ws-client/src/             # 浏览器/Node 可复用的 WS Client SDK
│   │   ├── gatewayClient.ts       # 请求封装、会话/聊天/工具/审批 API
│   │   ├── eventDispatcher.ts     # 事件分发 + chat.delta 批处理
│   │   ├── requestManager.ts      # requestId、超时、幂等键
│   │   └── resumeManager.ts       # 断线恢复的 session/seq 跟踪
│   ├── storage/src/
│   │   └── db.ts                  # SQLite 数据库（FTS5 + 向量表）
│   └── audit/
│       └── auditLogger.ts         # 审计日志
├── tests/                         # 400 个测试，42 suites，0 失败
...
npm run web:build           # 构建本地 Web UI
npm run gateway:smoke:ws    # WS 冒烟测试
npm run typecheck           # TypeScript 类型检查
npm test                    # 运行全部测试（400 个测试，0 失败）
npm run build               # 构建项目
...
**协议：**
- 26 个请求方法：`connect`、`ping`、`runtime.status`、`session.*`、`chat.*`、`memory.*`、`mcp.*`、`skills.*`、`tool.*`、`approval.*`、`audit.tail`
- 20 个 server event：`connected`、`heartbeat`、`run.*`、`chat.*`、`tool.*`、`approval.*`、`session.updated`、`audit.append`、`server.shutdown`、`state.resync_required`
- `session.rename` 支持指定 `sessionId`，用于从 Web UI 重命名已有会话并通过 `session.updated` 同步所有订阅端。
- `session.bindProject` 支持将会话绑定到现有的非系统本机目录，绑定后该目录成为命令与文件工具的工作边界。
- `chat.delta` 提供真实流式增量；当模型提供商不支持流式时，前端会用最终文本做播放式降级展示。
...
**Run 生命周期：**
- `chat.send` → `run.started` → `run.progress`(N) → `chat.delta`(N) → `chat.completed` + `run.finished`
- `chat.cancel` → `run.cancelled`
- 断线恢复：`session.getTranscript`(afterSeq) + `audit.tail`(afterSeq) 补齐间隙

**前端实现：**
- `packages/ws-client`：统一请求、幂等键、重连、事件分发、delta 批处理、resume 状态。
- `apps/web-ui`：React + Zustand 控制台，展示流式输出、过程时间线、工具调用、审批、资源、记忆和审计页面。
...
- 支持创建、切换、列出、重命名会话
- 项目绑定：`bindProjectDir` 将会话绑定到特定项目目录，允许绑定任意现有非系统目录
- 前端重命名：左侧 session 项可内联编辑，保存后调用 `session.rename`，后端持久化 `name/displayName` 并广播 `session.updated`
- 自动压缩：超过阈值时自动压缩旧会话记录
...
### 2026-05-06（本地 Web UI + WS Client + 会话体验）

**Web UI 控制台：**
- 新增 `apps/web-ui` React 控制台，采用左侧功能区 + 最近会话列表 + 大聊天区 + 可收缩右侧事件卡片布局
- 聊天区展示流式增量、最终回答、运行过程、工具调用、工具输出和 DevTask/ReviewGraph 状态
- 运行配置、模型、流式能力、工具循环、多 Agent 开关和工作目录移至聊天顶部小字展示
- 右侧事件流默认收缩成卡片，按需展开查看完整实时生命周期
- 会话列表支持新建、搜索、切换、绑定工作目录和内联重命名

**WS Client SDK：**
- 新增 `packages/ws-client`，封装请求/响应、重连、幂等键、事件分发、`chat.delta` 批处理和 resume 状态
- Web UI 通过 SDK 订阅 `run.*`、`chat.*`、`tool.*`、`approval.*`、`session.updated` 等事件并同步 Zustand 状态

**Gateway / WS 协议：**
- `runtime.status` 返回模型、流式能力、自动工具循环、多 Agent 和沙箱根目录摘要
- `chat.send` 记录 transcript，广播 `run.progress`，并在 `chat.completed` 中返回清洗后的最终文本
- `session.rename` 支持指定已有 `sessionId`，同步更新 `name` 与 `displayName` 并广播 `session.updated`
- `session.bindProject` 放宽为任意现有非系统目录，同时继续通过 `pathGuard` 拦截敏感主机路径
- 新增 MCP 配置编辑、Skill 当前/启用/清空、审批列表/确认/拒绝、审计 tail 等 WS 方法

**验证：**
- `npm run typecheck`：通过
- `npm run web:build`：通过
- `npx tsx --test tests/wsRouter.test.ts`：16 个通过
- `npm test`：400 个通过，42 suites，0 失败
